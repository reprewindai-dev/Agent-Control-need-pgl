...
export class RealtimeScoringEngine extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      console.warn('RealtimeScoringEngine Kafka connection failed (running in simulation/fallback mode):', err instanceof Error ? err.message : err);
    }
  }
...
export const globalEngine = new RealtimeScoringEngine();
globalEngine.start().catch((err) => {
  console.warn('RealtimeScoringEngine global start warning (normal if Kafka is offline):', err.message);
});
...
export class ScoreStreamServer {
  ...
  constructor(engine: RealtimeScoringEngine = globalEngine, port: number = 8080) {
    this.engine = engine;
...
export function setupSseEndpoint(
  ...
  engine: RealtimeScoringEngine = globalEngine
): void {
...
export { ScoreUpdate, Measurement, ScoringTrigger };
